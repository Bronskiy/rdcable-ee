...
class Wygwam_ft extends EE_Fieldtype {
	...
	public function display_field($data)
	{
		...
		return '<div class="wygwam"><textarea id="'.$id.'" name="'.$this->field_name.'" rows="10" data-config="'.$configHandle.'" data-defer="'.(!empty($this->settings['defer']) && $this->settings['defer'] == 'y' ? 'y' : 'n').'">'.$data.'</textarea></div>'.$this->_generateAssetInputsString($assetInfo);
	}
	...
	{
		Helper::includeFieldResources();
		$configHandle = Helper::insertConfigJsById($this->settings['config_id']);
